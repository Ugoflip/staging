...
export async function selectWinners(
  ...
  function validateInitTransaction(transactionData, pubKey) {
    ...
    if (!validateSignature(pubKey, signature, [messageBuf])) {
      errorMessage("Failed to select winner due to mis matched data 11");
      removeLoading();
      return;
    }
     let initObject = JSON.parse(messageBuf.toString());
  ...
  function validateEndTransaction(
    ...
    let endObject = JSON.parse(messageBuf.toString());
